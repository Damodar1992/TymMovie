...
import { useGenresQuery, useMoviesQuery, type MovieStatus } from '../../api/lists';
...
type SortOpt = 'created_at' | 'watch_date' | 'rating';
...
export function MobileFiltersScreen({ open, onApply }: MobileFiltersScreenProps) {
  ...
  const { data: serverGenres = [] } = useGenresQuery(activeListId);
  ...
  const sortValue: SortOpt =
    sortBy === 'watch_date' || sortBy === 'rating'
      ? sortBy
      : 'created_at';
  ...
  return (
    <MobileBottomSheet
      ...
      onClose={onApply}
      title="Filters"
      sheetClassName="mobile-filters-sheet"
      bodyClassName="mobile-filters-sheet-body"
      footer={
        <ApplyFiltersCTA
    ...
    >
      <div className="filters-v2 mobile-filters-sheet-content">
        <FilterSectionCard
          title="Status"
          summary={
            status === 'WATCHED'
